...
namespace Yatzy
{
    public partial class Form1 : Form
    {
        ...
        private void BtnSlå_Click(object sender, EventArgs e) //Vid klick på knappen BtnSlå utförs detta
        {
            ...
            //Antal kast
           if ( raknare == 0) //Startar räknaren
            {
                lblSlag.Text = "0 slag, Första spelaren kan börja!"; //Printar i label
                raknare++; //lägger till 1 på räknaren
            }
            else if ( raknare == 1) // När räknaren är 1 printas det att man har slagit tärningen en gånger.
            {
                lblSlag.Text = "1 slag"; //Printar i label

                raknare++; //lägger till 1 på räknaren

                cB1.Checked = false; //Tar bort check från föregående spelare
                cB2.Checked = false; //Tar bort check från föregående spelare
                cB3.Checked = false; //Tar bort check från föregående spelare
                cB4.Checked = false; //Tar bort check från föregående spelare
                cB5.Checked = false; //Tar bort check från föregående spelare
            }
           else if ( raknare == 2) // När räknaren är 2 printas det att man har slagit tärningen två gånger.
            {
                lblSlag.Text = "2 slag"; //Printar i label

                raknare++; //lägger till 1 på räknaren
            }
            else if ( raknare == 3) // När räknaren är 3 printas det att man har slagit tärningen tre gånger.
            {
                lblSlag.Text = "3 slag, Nästa spelare!"; //Printar i label

                raknare = 1; //Återställer räknaren till 1st slag.
